...
#### A FAIRE : hypothèses du modèle linéaire
...
On cherche à approximer le nuage des points
...
```{r Graphique de la droite de regression avec IC}
# Fonction d'affichage de la droite de regression avec ses intervalles de confiance (95%) :
ggplot(df, aes(x=Superficie, y=Prix))+ 
  geom_point()+
  geom_smooth(method=lm, se=T)
```
...
```{r Graphiques pour l'analyse des residus}
# Graphique residus Vs Fitted :
autoplot(
  modele,
  which = 1,
  ncol = 1,
