...
Ce bloc-note reprend le contenu du cours STA102 du Conservatoire National des Arts et Métier (CNAM) sur les modéles linéaires. Son but est de décrire les étapes d'une analyse de la variance (ANOVA) à un facteur, en rassemblant à la fois le matériel théorique et pratique pour travailler efficacement sur des données.
...
- La moyenne de $Y$ pour l'ensemble des observations : $Y_{..}=\frac{1}{n}\sum_{i=1}^{K}\sum_{j=1}^{n_{i}}y_{ij}$
